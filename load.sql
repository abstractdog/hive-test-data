...
stored as textfile;
LOAD DATA INPATH '/tmp/sample.csv' OVERWRITE INTO TABLE sample;


CREATE TABLE IF NOT EXISTS sample_orc_tmp
(id int, desc string, name varchar(50), category int, price1 float, price2 double, vat decimal, region varchar (100), department varchar (100), ratio decimal) 
STORED AS ORC;
INSERT INTO TABLE sample_orc SELECT * FROM sample_txt_tmp;
...
INSERT INTO sample_orc_part PARTITION (region) SELECT id, desc, name, category, price1, price2, vat, department, ratio, region FROM sample_orc_tmp;
